...
class ImageProcessor {
	...
	async processImage(imagePath) {
		...
		try {
			...
			// Store image dimensions for HTML processor
			const dimensionsFile = path.join(optImagesDir, "dimensions.json");
			let dimensions = {};
			if (fs.existsSync(dimensionsFile)) {
				try {
					dimensions = JSON.parse(fs.readFileSync(dimensionsFile, "utf8"));
				} catch (error) {
					dimensions = {};
				}
			}
