...
class HtmlProcessor {
	...
	async createPictureElement(imgTag, htmlFile) {
		try {
			...
			for (const size of this.config.sizes) {
				...
				if (webpVersion) {
					// Convert to absolute path for web
					const absolutePath = "/" + path.relative("public", webpVersion);
					const source = parse(
						`<source media="(min-width:${size}px)" srcset="${absolutePath}"></source>`,
	...
	async getWebpVersions(originalImagePath) {
		try {
			...
			// Look for WebP versions in the public folder
			let publicDir;
			if (originalImagePath.startsWith("source/")) {
				publicDir = path.join("public", path.relative("source", dir));
			} else if (originalImagePath.startsWith("themes/")) {
				// For theme images, look in public folder with same relative path
				const themeRelativePath = path.relative("themes", originalImagePath);
				publicDir = path.join("public", path.dirname(themeRelativePath));
			} else {
				// For images already in public folder
				publicDir = dir;
			}

			const webpPattern = path.join(publicDir, `${baseName}*.webp`);
			const webpFiles = glob.sync(webpPattern, { nodir: true });

			return webpFiles;
		} catch (error) {
			this.logger.error(`Error getting WebP versions: ${error.message}`);
			return [];
		}
	}
